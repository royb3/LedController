...
void loop() {
  ...
  while(remaining > 0) { // While more LED data is expected...
    ...
    if((c = Serial.read()) >= 0) {    // Successful read?
      ...
      if(timeout(t, 50) == true) return; // Start over
    }
  }
...
static boolean timeout(
  ...
  if((t - lastByteTime) > serialTimeout) {
    long bytes = 50 * 3L;
    FastSPI_LED.show();  // Latch any partial/incomplete data in strand
    for(int i=0; i < 50; i++)
    {
       leds[i].r=0;
       leds[i].g = 0;
    ...
    FastSPI_LED.show();     // Latch 'all off' data
